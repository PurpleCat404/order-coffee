document.addEventListener('DOMContentLoaded', function() {
	...
	function getNotesText(fieldset) {
		const notesArea = fieldset.querySelector('.user-notes');
		return notesArea ? notesArea.value.trim() : '';
	}
	...
	function createOrderTable() {
      ...
      `;
		...
		fieldsets.forEach(fieldset => {
			...
			const notes = getNotesText(fieldset);
	...
	function openModal() {
      ...
      `;
		...
		confirmButton.addEventListener('click', function() {
			...
			if (selH < nowH || (selH === nowH && selM <= nowM)) {
				timeInput.style.border = '1px solid red';
	...
	addButton.addEventListener('click', function() {
		...
		const notesArea = newBeverage.querySelector('.user-notes');
